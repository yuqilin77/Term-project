...
document.getElementById("myForm").addEventListener("submit", function(e) {
    ...
    // prevent the submit event happens
      ...
      // extract the first name, last name and facilitor's first name entered by the users
      const firstName = document.getElementById("firstName").value;
      const lastName = document.getElementById("lastName").value;
      const email = document.getElementById("email").value;
      const number = document.getElementById("number").value;
  
      // define the name Regex for valid entry
      const nameRex = /^[A-Za-z]{2,}$/;
      const emailRex = /^[\w.-]+@[a-zA-Z_-]+?\.[a-zA-Z]{2,3}$/;
      const phoneRex = /^\d{10}$/;
    
      // test if the first name and last name agrees to the Regex
